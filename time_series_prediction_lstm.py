...
if __name__ == "__main__":
    ...
    training_data = load_data(input_variables, output_variable)
    ...
    model.add(layers.LSTM(32, activation='relu', input_shape=(X_train.shape[1], X_train.shape[2]), return_sequences=False))
    model.add(layers.Dropout(0.2))
    model.add(layers.Dense(1))
    model.compile(optimizer='adam', loss='mse')
    print(model.summary())
    hist = model.fit(X_train, Y_train, epochs=100, batch_size=25, validation_split=0.1, verbose=1)
